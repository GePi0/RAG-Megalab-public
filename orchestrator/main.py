...
def handle_prompt(body: PromptRequest):
    """
    Flujo principal de ejecución del Orchestrator:
      1️⃣ Recibe prompt y mantiene contexto.
      2️⃣ Aplica políticas adaptativas y estrategias.
      3️⃣ Ejecuta Llama 3.1 adaptativo.
      4️⃣ Delegación al Worker MCP + almacenamiento en Chroma.
      5️⃣ Ejecuta Supervisor + Meta‑Reflexión.
    """
    ...
    try:
        ...
        # ----------------------------------------------------- 2.5️⃣ Context Awareness 2.0
        from context_observer import scan_project

        project_root = f"/workspace/projects/{project_id}"
        awareness = scan_project(project_root)
        print(
            f"🧩 Context Awareness → {project_id}: "
            f"{len(awareness['files'])} archivos detectados"
        )
        ...
        log_event(
            ...
            "INFO",
            "llama_invoke",
            "Ejecutando cadena Llama 3.1 adaptativo",
            task_id=task_id,
        )
        ...
        # ----------------------------------------------------- 7️⃣ Supervisor + Meta‑Reflexión (asíncrono)
...
def startup_event():
    """Arranque de Health Monitor + Feedback + Healing."""
    import threading
    ...
    print("🩺 Health Manager, Feedback y Healing Listener activos (thread daemon).")
